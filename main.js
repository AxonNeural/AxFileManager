...
const total_files = fs.readdirSync('./storage/partitions').filter(filename => filename.endsWith('.json')).length;
if (total_files === 0) {
    _cache.current_file = 1;
    fs.writeFileSync('./storage/partitions/1.json', JSON.stringify([]), { encoding: 'utf-8' });
} else {
    const file_size = fs.statSync(`./storage/partitions/${total_files}.json`).size / 1024;
    _cache.current_file = total_files;
    if (file_size > 100) {
        _cache.current_file++;
        fs.writeFileSync(`./storage/partitions/${_cache.current_file}.json`, JSON.stringify([]), { encoding: 'utf-8' });
    } else {
        _cache.file_content = require(`./storage/partitions/${_cache.current_file}.json`);
    }
}
...
exports.addInMemory = (object, tags) => {
    ...
    for (const tag of tags) {
        if (pointer[tag]) {
            if (pointer[tag][_cache.current_file]) {
                let convertIntoArray = revertRanges(pointer[tag][_cache.current_file]);
                if (!convertIntoArray.includes(object.ID)) {
                    ...
                    pointer[tag][_cache.current_file] = convertIntoString;
                };
            } else {
                pointer[tag][_cache.current_file] = `${object.ID}`;
            }
        } else {
            pointer[tag] = {};
            pointer[tag][_cache.current_file] = `${object.ID}`;
    ...
    if (new_size > 100) {
        fs.writeFile(`./storage/partitions.pointer.json`, JSON.stringify(pointer), { encoding: 'utf-8' }, () => {});
        fs.writeFile(`./storage/partitions/${_cache.current_file}.json`, JSON.stringify(_cache.file_content), { encoding: 'utf-8' }, () => {});
        _cache.current_file++;
        _cache.file_content = [];
    }
}
